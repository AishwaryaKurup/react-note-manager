...
import { t as _extends$1 } from "./extends-VVEeICP7.js";
//#region node_modules/history/index.js
...
function createBrowserHistory(options) {
	...
	function handlePop() {
		if (blockedPopTx) {
			...
			} else warning$2(false, "You are trying to block a POP navigation to a location that was not created by the history library. The block will fail silently in production, but in general you should do all navigation with the history library (instead of using window.history.pushState directly) to avoid this situation.");
...
function createHashHistory(options) {
	if (options === void 0) options = {};
	var _options2$window = options.window, window = _options2$window === void 0 ? document.defaultView : _options2$window;
	var globalHistory = window.history;
	function getIndexAndLocation() {
	...
	window.addEventListener(HashChangeEventType, function() {
		var nextLocation = getIndexAndLocation()[1];
	...
	function push(to, state) {
		...
		warning$2(nextLocation.pathname.charAt(0) === "/", "Relative pathnames are not supported in hash history.push(" + JSON.stringify(to) + ")");
		if (allowTx(nextAction, nextLocation, retry)) {
	...
	function replace(to, state) {
		...
		warning$2(nextLocation.pathname.charAt(0) === "/", "Relative pathnames are not supported in hash history.replace(" + JSON.stringify(to) + ")");
		if (allowTx(nextAction, nextLocation, retry)) {
...
function createMemoryHistory(options) {
	...
	var entries = initialEntries.map(function(entry) {
		...
		warning$2(location.pathname.charAt(0) === "/", "Relative pathnames are not supported in createMemoryHistory({ initialEntries }) (invalid entry: " + JSON.stringify(entry) + ")");
		return location;
	});
	...
	function push(to, state) {
		...
		warning$2(location.pathname.charAt(0) === "/", "Relative pathnames are not supported in memory history.push(" + JSON.stringify(to) + ")");
		if (allowTx(nextAction, nextLocation, retry)) {
			index += 1;
	...
	function replace(to, state) {
		...
		warning$2(location.pathname.charAt(0) === "/", "Relative pathnames are not supported in memory history.replace(" + JSON.stringify(to) + ")");
		if (allowTx(nextAction, nextLocation, retry)) {
...
//#endregion
//#region node_modules/react-router/index.js
/**
* React Router v6.3.0
*
...
function compilePath(path, caseSensitive, end) {
	...
	} else regexpSource += end ? "\\/*$" : "(?:(?=[.~-]|%[0-9A-F]{2})|\\b|\\/|$)";
...
function useNavigate() {
	...
	return (0, import_react.useCallback)(function(to, options) {
		if (options === void 0) options = {};
...
function useOutlet(context) {
	...
	if (outlet) return /*#__PURE__*/ (0, import_react.createElement)(OutletContext.Provider, { value: context }, outlet);
	return outlet;
}
/**
...
function useResolvedPath(to) {
	...
	let { pathname: locationPathname } = useLocation();
	let routePathnamesJson = JSON.stringify(matches.map((match) => match.pathnameBase));
	return (0, import_react.useMemo)(() => resolveTo(to, JSON.parse(routePathnamesJson), locationPathname), [
		to,
		routePathnamesJson,
		locationPathname
	]);
...
function Navigate(_ref2) {
	...
	warning$1(!(0, import_react.useContext)(NavigationContext).static, "<Navigate> must not be used on the initial render in a <StaticRouter>. This is a no-op, but you should modify your code so the <Navigate> is only ever rendered in response to some user interaction or state change.");
...
function Router(_ref3) {
	...
	return /*#__PURE__*/ (0, import_react.createElement)(NavigationContext.Provider, { value: navigationContext }, /*#__PURE__*/ (0, import_react.createElement)(LocationContext.Provider, {
		children,
		value: {
			location,
			navigationType
...
function createRoutesFromChildren(children) {
	...
	import_react.Children.forEach(children, (element) => {
		if (!/*#__PURE__*/ (0, import_react.isValidElement)(element)) return;
...
//#endregion
//#region node_modules/react-router-dom/index.js
/**
...
function BrowserRouter(_ref) {
	...
	let historyRef = (0, import_react.useRef)();
	if (historyRef.current == null) historyRef.current = createBrowserHistory({ window });
	let history = historyRef.current;
	let [state, setState] = (0, import_react.useState)({
		action: history.action,
		location: history.location
...
function HashRouter(_ref2) {
	...
	let historyRef = (0, import_react.useRef)();
	if (historyRef.current == null) historyRef.current = createHashHistory({ window });
	let history = historyRef.current;
	let [state, setState] = (0, import_react.useState)({
		action: history.action,
		location: history.location
...
*/
var Link = /*#__PURE__*/ (0, import_react.forwardRef)(function LinkWithRef(_ref4, ref) {
	...
	return /*#__PURE__*/ (0, import_react.createElement)("a", _extends({}, rest, {
		href,
		onClick: handleClick,
		ref,
		target
...
*/
var NavLink = /*#__PURE__*/ (0, import_react.forwardRef)(function NavLinkWithRef(_ref5, ref) {
	...
	let className;
	if (typeof classNameProp === "function") className = classNameProp({ isActive });
	else className = [classNameProp, isActive ? "active" : null].filter(Boolean).join(" ");
	let style = typeof styleProp === "function" ? styleProp({ isActive }) : styleProp;
	return /*#__PURE__*/ (0, import_react.createElement)(Link, _extends({}, rest, {
...
function useLinkClickHandler(to, _temp) {
	...
	return (0, import_react.useCallback)((event) => {
		if (event.button === 0 && (!target || target === "_self") && !isModifiedEvent(event)) {
			event.preventDefault();
			navigate(to, {
				replace: !!replaceProp || createPath(location) === createPath(path),
				state
			});
		}
	}, [
...
//#endregion
export { BrowserRouter, HashRouter, Link, MemoryRouter, NavLink, Navigate, Action as NavigationType, Outlet, Route, Router, Routes, LocationContext as UNSAFE_LocationContext, NavigationContext as UNSAFE_NavigationContext, RouteContext as UNSAFE_RouteContext, createPath, createRoutesFromChildren, createSearchParams, generatePath, matchPath, matchRoutes, parsePath, renderMatches, resolvePath, HistoryRouter as unstable_HistoryRouter, useHref, useInRouterContext, useLinkClickHandler, useLocation, useMatch, useNavigate, useNavigationType, useOutlet, useOutletContext, useParams, useResolvedPath, useRoutes, useSearchParams };
